package com.carlt.chelepie.http;
...
public class HttpConnector {
	 ...
	 * @return 连接结果
	 */
	public int connect(String url, byte[] data, String startPos, String endPos) {
		...
		try {
			for (int i = 0; i < retryNum; i++) {
				...
				if (startPos != null) {
					...
					connection.setRequestProperty("Range", sProperty.toString());
				}
				connection.setRequestProperty("Accept", "*/*");
				// 设置HTTP请求方法
				connection.setRequestMethod(requestMethod);
				// post数据
				if (requestMethod.equals(METHOD_POST) && data != null) {
				...
				if (responseCode == HttpURLConnection.HTTP_OK || responseCode == HttpURLConnection.HTTP_PARTIAL) {
					result = CONNECT_OK;
					return result;
				} else if (responseCode == HttpURLConnection.HTTP_MOVED_TEMP || responseCode == HttpURLConnection.HTTP_MOVED_PERM) {// 跳转

				} else if (responseCode == HttpURLConnection.HTTP_FORBIDDEN || responseCode == HttpURLConnection.HTTP_NOT_FOUND) {
					// 这种情况表示服务不可用，无需重试
					connection.disconnect();
					connection = null;
					result = CONNECT_UNAVAILABLE;
	...
	public int connect(String url, byte[] data) {
		return connect(url, data, null, null);
	}
