...
import com.carlt.doride.utils.MiscUtil;
...
public class CircleProgress extends View {
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mMaxValue = typedArray.getFloat(R.styleable.CircleProgressBar_maxProgress, DEFAULT_MAX_VALUE);
        //内容数值精度格式
    ...
    private void drawText(Canvas canvas) {
        ...
        canvas.drawText(packageDateParse(valueTxt), mCenterPoint.x, mValueOffset, mValuePaint);
    ...
    private void drawArc(Canvas canvas) {
        ...
        canvas.drawArc(mRectF, -90, 360, false, mBgArcPaint);
        ...
        if (currentAngle>0) {
            canvas.drawArc(mRectF, -90, currentAngle, false, mArcPaint);
        }
        canvas.restore();
    }
    ...
    public void setValue(float value) {
        ...
        float end=0.0f;
        if (mMaxValue == 0 || Float.isNaN(mMaxValue)) {
            end = 0.0f;
        } else if (value < 0) {
            end = 0.0f;
        } else {
