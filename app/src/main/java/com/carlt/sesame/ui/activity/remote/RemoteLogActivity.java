...
public class RemoteLogActivity extends LoadingActivityWithTitle {
    ...
    private ArrayList<RemoteLogInfo> mList = new ArrayList<RemoteLogInfo>();// 数据list
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mDControl = DaoControl.getInstance();
    ...
    private void init() {
        mPullListView = (PullToRefreshListView) findViewById(R.id.remotelog_list);
        mTxtEmpty = (TextView) findViewById(R.id.remotelog_txt_empty);
    ...
    private void PullDown() {
        // todo 调用接口
        CPControl.GetRemoteLog(LIMIT, 0, listener);
    }
    ...
    private void PullUp() {
        ...
        CPControl.GetRemoteLog(LIMIT, offset, listener_loadmore);
    }

    @Override
    protected void LoadSuccess(Object data) {
        mInfoLists = (RemoteLogListInfo) data;
        if (mInfoLists != null) {
            mList = mInfoLists.getmRemoteLogInfoList();
            if (mAdapter == null) {
                mAdapter = new RemoteLogAdapter(RemoteLogActivity.this, mList);
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 0:
                    RemoteLogListInfo mMore = (RemoteLogListInfo) msg.obj;
                    if (mInfoLists != null) {
                        mInfoLists.setOffset(mMore.getOffset());
                        mInfoLists.addmRemoteLogInfoList(mMore.getmRemoteLogInfoList());
                        if (mMore.getmRemoteLogInfoList().size() == 0) {
