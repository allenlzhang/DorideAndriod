...
public class UUChargeDialog extends Dialog implements OnClickListener,
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.charge_txt_confirm:
                ...
                //                                        listener_charge);
                LogUtils.e(chargeTime.toString());
                long time = MyTimeUtil.date2TimeStamp(chargeTime.toString());

                remoteCharge(optType, time);
    ...
    private void remoteCancelCharge() {
        ...
        map.put("optType", 4);
        ApiRetrofit.getInstance().getApiService().RemoteCancelCharge(map)
                ...
                .subscribe(result -> {
                    ...
                    UUToast.showUUToast(mContext, "操作失败");
                });
    }

    @SuppressLint("CheckResult")
    private void remoteCharge(int optType, long chargeTime) {
        HashMap<String, Object> map = new HashMap<>();
        map.put("moveDeviceName", DorideApplication.MODEL_NAME);
        map.put("base", ApiRetrofit.getRemoteCommonParams());

        LogUtils.e(chargeTime);
        map.put("optType", optType);
        if (chargeTime != -1) {
            map.put("chargeTime", chargeTime);
        }

        ApiRetrofit.getInstance().getApiService().RemoteCharge(map)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(result -> {
                    Message message = mHandler.obtainMessage();
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    ...
                    RemoteCommonInfo commonInfo = (RemoteCommonInfo) msg.obj;
                    // 设置充电成功
                    if (commonInfo.err == null) {
                        if (TextUtils.isEmpty(commonInfo.msg)) {
                            UUToast.showUUToast(mContext, "操作成功");
                            switch (currentIndex) {
                                case 0:
                                    // 立即充电操作成功
                                    dismiss();
                                ...
                                case 1:
                                    ...
                                    chargeTimeResult = chargeTime.toString();
                                    isChargeTimered = true;
                                    setCurrentView(1);
                                    break;
                                case 2:
                            ...
                            UUToast.showUUToast(mContext, commonInfo.msg);
                        }
                    } else {
                        date = "";
                        hour = "";
                        ...
                        UUToast.showUUToast(mContext, commonInfo.err.msg);
                    }
                ...
                case 1:
                    ...
                    break;
                case 2:
                    RemoteCommonInfo commonInfo1 = (RemoteCommonInfo) msg.obj;
                    // 取消定时充电成功
                    if (commonInfo1.err == null) {
                        if (TextUtils.isEmpty(commonInfo1.msg)) {
                            UUToast.showUUToast(mContext, "操作成功");
                        } else {
                            UUToast.showUUToast(mContext, commonInfo1.msg);
                        }
                    } else {
                        UUToast.showUUToast(mContext, commonInfo1.err.msg);
                    }
                    dismiss();

                    //                    BaseResponseInfo mInfo1 = (BaseResponseInfo) msg.obj;
                    //                    // 取消定时充电成功
                    //                    if (!TextUtils.isEmpty(mInfo1.getInfo())) {
                    //                        info_show = mInfo1.getInfo();
                    //                    } else {
                    //                        info_show = "操作成功！";
                    //                    }
                    //                    UUToast.showUUToast(mContext, info_show);
                ...
                case 3:
                    ...
                    // 取消定时充电失败
                    if (!TextUtils.isEmpty(mInfo2.getInfo())) {
                        info_show = mInfo2.getInfo();
                    } else {
                        info_show = "操作失败...";
                    }
                    break;
