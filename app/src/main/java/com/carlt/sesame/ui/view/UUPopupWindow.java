...
public class UUPopupWindow extends PopupWindow {
	...
	private OnScrollChangedListener mOnScrollChangedListener = new OnScrollChangedListener() {
		public void onScrollChanged() {
			...
			if (anchor != null && mPopupView != null) {
				...
				updateAboveAnchor(findDropDownPosition(anchor, p, mAnchorXoff,
						mAnchorYoff));
	...
	public UUPopupWindow(Context context) {
		super(context);
	}

	public UUPopupWindow(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public UUPopupWindow(Context context, AttributeSet attrs, int defStyle) {
	...
	public UUPopupWindow(View contentView) {
		super(contentView);
	}

	public UUPopupWindow(int width, int height) {
		super(width, height);
	}

	public UUPopupWindow(View contentView, int width, int height) {
	 ...
	 * a focusable {@link View}. By default a popup window is not
	 * focusable.
	 * </p>
	 *
	 * <p>
	 ...
	 * {@link WindowManager.LayoutParams} for more information on
	 * how gravity and the x and y parameters are related. Specifying a gravity
	 * of {@link Gravity#NO_GRAVITY} is similar to specifying
	...
	private void updateAboveAnchor(boolean aboveAnchor) {
		if (aboveAnchor != mAboveAnchor) {
			...
			if (mBackground != null) {
				...
				// with above-anchor
				// and below-anchor states, use those. Otherwise rely on
				// refreshDrawableState to
				// do the job.
	...
	private int computeAnimationResource() {/*
											 * if (mAnimationStyle == -1) { if
											 * (mIsDropdown) { return
											 * mAboveAnchor ?
											 * com.android.internal
											 * .R.style.Animation_DropDownUp :
											 * com.android.internal.R.style.
											 * Animation_DropDownDown; } return
											 * 0; } return mAnimationStyle;
											 */
		return 0;
	}
	 ...
	 */
	private boolean findDropDownPosition(View anchor,
		...
		if (p.y + mPopupHeight > displayFrame.bottom
			...
			onTop = (displayFrame.bottom - mScreenLocation[1]
					- anchor.getHeight() - yoff) < (mScreenLocation[1] - yoff - displayFrame.top);
			if (onTop) {
	...
	// Ϊʵ�ֹرն�����д���ִ���
	private boolean ani2IsRunning = false;

	private Animation ani2;
	private View menuView;

	public void initAni(View menuView, Animation ani2) {
