...
public class PickerView extends View {
    ...
    public PickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        linePaint = new Paint();
        linePaint.setColor(Color.parseColor("#5ca7d7"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (null != dataList && dataList.size() > 0) {
            ...
            int ys = cy - centerPadding / 2;
            int yx = cy + centerPadding / 2;

            int xs = cx - contentWidth / 2;
            int xe = cx + contentWidth / 2;
            //画线
            //canvas.drawLine(xs, ys, xe, ys, linePaint);
            //canvas.drawLine(xs, yx, xe, yx, linePaint);
        }
    }
